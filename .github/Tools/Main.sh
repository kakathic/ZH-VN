...
[ -z "$imei" ] && imei="Hãy ấn *#06# để hiện MEID)"

driver=$(getprop ro.product.vendor.device)
[ "$driver" ] || driver=$(getprop ro.product.system.device)

Xem "https://raw.githubusercontent.com/kakathic/ZH-VN/ZH/Code/$driver/$(echo "$Apilt" | base64 -w0)" > $TMPDIR/acc.sh
Xem "https://raw.githubusercontent.com/kakathic/ZH-VN/ZH/Code/$driver/$(echo "$imei" | base64 -w0)" > $TMPDIR/acc2.sh

chmod -R 777 $TMPDIR/*.sh
Pro(){
[ "$(grep -c '#kakathic' $TMPDIR/acc.sh)" == 1 ] && . $TMPDIR/acc.sh
}
Pro2(){
[ "$(grep -c '#kakathic' $TMPDIR/acc2.sh)" == 1 ] && . $TMPDIR/acc2.sh
}

if [ "$(Pro)" == 1 ] || [ "$(Pro2)" == 1 ];then

ui_print "  Chào bạn: $Apilt"
ui_print
ui_print "  Sử dụng đến: $Dtime"
ui_print
elif [ -e /data/tools/lib/log.txt ];then
abort "! Bạn đã hết thời gian dùng thử
  
  Mời bạn ủng hộ để tiếp tục sử dụng.
"
else
ui_print "! Thông báo ủng hộ 
...
Apilt='$Apilt'
imei='$imei'
curl -#G "https://raw.githubusercontent.com/kakathic/ZH-VN/ZH/Code/$driver/$(echo "$Apilt" | base64 -w0)" > /data/tools/lib/acc.sh
curl -#G "https://raw.githubusercontent.com/kakathic/ZH-VN/ZH/Code/$driver/$(echo "$imei" | base64 -w0)" > /data/tools/lib/acc2.sh
chmod -R 777 /data/tools/lib/*.sh
Pro(){
[ "$(grep -c '#kakathic' /data/tools/lib/acc.sh)" == 1 ] && . /data/tools/lib/acc.sh
}
Pro2(){
[ "$(grep -c '#kakathic' /data/tools/lib/acc2.sh)" == 1 ] && . /data/tools/lib/acc2.sh
}
if [ "$(Pro)" == 1 ] || [ "$(Pro2)" == 1 ];then
